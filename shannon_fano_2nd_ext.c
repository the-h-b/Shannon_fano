...
typedef struct {
    char symbols[3]; // Maximum length for a combination of two symbols
    float probability;
    char code[MAX_SIZE];
} CodeWord;
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            snprintf(words[index].symbols, 3, "%s%s", symbols[i], symbols[j]);
            words[index].probability = probabilities[i] * probabilities[j];
